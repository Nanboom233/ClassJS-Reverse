...
var unpackWebpackModules = (sourceCode) => {
    ...
    // Complete removal of the assignment
    if (dictPathToRemove) {
        if (dictPathToRemove.parentPath && dictPathToRemove.parentPath.node && dictPathToRemove.parentPath.node.declarations && dictPathToRemove.parentPath.node.declarations.length === 1) {
        ...
        CallExpression(path) {
            if (t.isFunctionExpression(path.node.callee) && path.node.arguments.length === 2) {
                if (t.isFunctionExpression(path.node.arguments[1])) {
                    iifeBody = path.node.arguments[1].body.body;
                    path.stop();
                }
            }
        }
    ...
    if (iifeBody) {
        // Prepend module functions
        for (let i = moduleFunctions.length - 1; i >= 0; i--) {
            iifeBody.unshift(moduleFunctions[i]);
        }
    }
