...
var unpackWebpackModules = (sourceCode) => {
                ...
                for (let prop of p.node.init.properties) {
                    if (t.isObjectProperty(prop)) {
                        ...
                        if (modName && t.isFunctionExpression(funcExpr)) {
                            // Name the function wrapperDictName_modName
                            let safeName = \`\${wrapperDictName}_\${modName}\`;
    ...
    // We will find the Program or block statement where the loader function resides.
    // That is the safe place to inject the new functions so they are globally available 
    // inside the Webpack runtime closure.
    let loaderPath = null;
    traverse(ast, {
        FunctionDeclaration(p) {
    ...
    if (loaderPath && wrapperDictName) {
        ...
        // Insert functions into the block containing the loader function
        if (loaderPath.parentPath && t.isBlockStatement(loaderPath.parentPath.node)) {
            loaderPath.parentPath.node.body.unshift(proxyFunc, ...moduleFunctions);
        } else if (loaderPath.parentPath && t.isProgram(loaderPath.parentPath.node)) {
            loaderPath.parentPath.node.body.unshift(proxyFunc, ...moduleFunctions);
        }
    }

    if (dictPathToRemove) {
        if (dictPathToRemove.parentPath && dictPathToRemove.parentPath.node.declarations && dictPathToRemove.parentPath.node.declarations.length === 1) {
            dictPathToRemove.parentPath.remove();
        } else {
...
// Replace the old unpackWebpackModules logic
let newCode = code.replace(/var unpackWebpackModules = \(sourceCode\) => \{[\s\S]*?\n\}\n\nfunction main\(\) \{/, newStep + '\nfunction main() {');

if (!newCode.includes('var unpackWebpackModules')) {
    newCode = code.replace('function main() {', newStep + '\nfunction main() {');
}
